...
# deploy-repo — full repo deploy

## Modes

| Mode | Invocation | Effect |
|------|-----------|--------|
| clone | `@deploy-repo clone - <url> <target>` | Git clone the framework repo |
| archive | `@deploy-repo archive - <dir>` | Create a compressed archive of the framework |

## Parse

```text
@deploy-repo clone - <url> <target-path> [--branch <branch>]
@deploy-repo archive - <output-dir> [--format zip|tar.gz]
```
...
6. Scaffold `.training.mlt/` skeleton in target if not present
...
4. Exclude `.training.mlt/` learner memory, `.git/`, `__pycache__/`, `node_modules/`
...
- clone: target directory contains full framework, `.training.mlt/` scaffolded, commit SHA reported
