...
usage() {
    echo "Usage: bash scripts/deploy-basic.sh <target-path> [--update] [--force]"
    echo "  <target-path>  Target project root (created if missing)"
    echo "  --update       Re-sync .training.mlt/ skeleton, preserve learner memory and .cursorrules"
...
if [ -f "$CURSORRULES" ]; then
    if [ "$FORCE" -eq 1 ]; then
        ...
        echo "  [SKIP] .cursorrules exists — preserved (--update never overwrites)"
        echo "         Merge new rules manually, or re-run with --force after confirmation."
    else
